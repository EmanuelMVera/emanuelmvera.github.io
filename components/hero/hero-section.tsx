    ...
    >
      ...
      <div className="relative grid items-center gap-12 lg:grid-cols-[1.1fr_1fr]">
        <div className="space-y-6">
          <p className="inline-flex rounded-full border border-(--border) bg-(--surface) px-3 py-1 font-mono text-xs font-semibold text-(--accent)">
            {hero.eyebrow}
          </p>
          <div className="space-y-2">
            ...
            <p className="text-lg font-semibold text-(--accent)">{hero.role}</p>
          </div>
          <p className="max-w-xl text-lg text-(--text)">{hero.tagline}</p>
          ...
          <div className="inline-flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1.5 text-xs text-emerald-700 dark:text-emerald-300">
            <span
              ...
              aria-hidden="true"
            />
            {siteConfig.ui.hero.opportunityBadge}
          </div>
          <HeroCtaGroup />
          <div className="flex flex-wrap gap-2" aria-label="Tecnologías principales">
            {hero.techPills.map((tech) => (
              <span
                key={tech}
                className="rounded-full border border-(--border) bg-(--surface) px-3 py-1 text-xs font-medium text-(--text) shadow-sm"
              >
                {tech}
              </span>
            ))}
